...
- The capability `CAP_NET_BIND_SERVICE` was removed from the module {option}`services.murmur` as it's only needed for privileged ports (lower than 1024) and there is not
  much reason to use these. So it's rather unusual to enable this capability by default, but users can still enable it in their configuration.
